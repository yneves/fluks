...
var Store = factory.createClass({
  ...
  constructor: function(dispatcher) {
    ...
    this.on("mount", this.storeWillMount);
    this.on("unmount", this.storeWillUnmount);
    ...
    0: function() {
      ...
      return {
        ...
        componentWillMount: function() {
          this._storeChangeHandler = function() {
            if (this.storeDidChange) {
              this.storeDidChange(store);
            }
          }.bind(this);
          store.addListener("change", this._storeChangeHandler);
          store.props = this.props;
          store.emit("mount");
        },
        
        componentWillUnmount: function() {
          store.removeListener("change", this._storeChangeHandler);
          store.props = undefined;
          store.emit("unmount");
        }
      };
    }
