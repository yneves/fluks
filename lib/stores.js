...
var Store = factory.createClass({
  ...
  constructor: function (dispatcher) {
    ...
    if (factory.isFunction(this.storeWillMount)) {
      var storeWillMountTrace = new Error(this.constructor.name + ': storeWillMount handler error');
      this.on('storeWillMount', this.catchError(this.storeWillMount, storeWillMountTrace));
    }

    if (factory.isFunction(this.storeWillUnmount)) {
      var storeWillUnmountTrace = new Error(this.constructor.name + ': storeWillUnmount handler error');
      this.on('storeWillUnmount', this.catchError(this.storeWillUnmount, storeWillUnmountTrace));
    }

    if (factory.isFunction(this.storeWillReceiveProps)) {
      var storeWillReceivePropsTrace = new Error(this.constructor.name + ': storeWillReceiveProps handler error');
      this.on('storeWillReceiveProps', this.catchError(this.storeWillReceiveProps, storeWillReceivePropsTrace));
    }
  },
    ...
    o: function (options) {
      ...
      return {
        ...
        componentWillMount: function () {
          storeChangeHandler = store.onChange('change', function () {
